...
export default function Scan() {
  ...
  const fetchData = async () => {
    const querySnapshot = await firebase.firestore().collection("items")
      .where("serialNo", "==", scannedData).get();
      ...
      <View style={styles.box}>
        <CameraView
          ...
          barcodeScannerSettings={{
            barcodeTypes: [
              "qr", "aztec", "codabar", "code128", "code39", "code93",
              "datamatrix", "ean13", "ean8", "itf14", "pdf417", "upc_a", "upc_e",
            ],
          }}
        ...
        {items.map((item) => (
          <View key={item.id} style={styles.modalContainer}>
            <View style={styles.modalView}>
              ...
              <Text style={styles.productPrice}>{"Rs. " + item.data.price + " / item"}</Text>
              <Text style={styles.quantityText}>Change the quantity you wish to add.</Text>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: "#A2D68F", justifyContent: "center", alignItems: "center" },
  box: { width: "85%", height: "25%" },
  camera: { width: "100%", flex: 1, position: "relative" },
  ...
  productPrice: { fontSize: 16, marginBottom: 20 },
  quantityText: { fontSize: 14, marginBottom: 10 },
  quantityContainer: { flexDirection: "row", alignItems: "center", marginBottom: 20 },
  button: { width: 40, height: 40, borderRadius: 20, backgroundColor: "#D3D3D3", justifyContent: "center", alignItems: "center" },
  buttonText: { fontSize: 20, fontWeight: "bold" },
  quantityValue: { marginHorizontal: 20, fontSize: 18 },
  modalButtons: { flexDirection: "row", justifyContent: "space-between", width: "100%" },
  cancelButton: { flex: 1, paddingVertical: 10, justifyContent: "center", alignItems: "center", backgroundColor: "#F0F0F0", marginRight: 10, borderRadius: 5 },
  addButton: { flex: 1, paddingVertical: 10, justifyContent: "center", alignItems: "center", backgroundColor: Colors.Primary, borderRadius: 5 },
  cancelText: { fontSize: 16, color: "#000" },
  addText: { fontSize: 16, color: "#FFF" },
});
